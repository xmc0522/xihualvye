...
import { tableData as DEFAULT_TABLE_DATA } from './天枢款-双面门-主表格数据'
import { allAccessories as DEFAULT_ACCESSORIES } from './天枢款-双面门-配件数据'
import { doorPanelRows as DEFAULT_DOOR_PANELS } from './天枢款-双面门-底部门板数据'
  ...
  const filteredTableData = computed(() => {
    ...
    for (let i = 0; i < result.length; i++) {
      if (result[i] && result[i]!.mingcheng === '加固') {
        result[i]!.guige = info.height ? String(Number(info.height) - 90) : ''
      }
    }
